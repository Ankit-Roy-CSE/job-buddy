...
JobBuddy is a full stack AI-powered job hunting assistant, career coach, and job preparation platform. 

The user sets up their profile once, uploads their resume, and the agent automatically discovers relevant jobs from Adzuna — scoring each one against the user's profile using GPT-4o. 

For jobs they're interested in, the agent researches the company across their public web pages and builds a structured dossier. Once applied and preparing for interviews, JobBuddy generates personalized interview questions and answers based on their role and experience using the Gemini API. Users can view Q&A in an expandable accordion UI, request dynamic AI-generated concept breakdowns, take notes, and pin important questions for quick access.
...
Job hunting and interview preparation are some of the most repetitive and time-consuming tasks a developer faces. Reading dozens of job descriptions, deciding if a role fits, researching each company from scratch, and endlessly searching for the right interview questions to practice — all of this is exhausting.

JobBuddy eliminates all of that preparation work. The agent finds the jobs, scores them intelligently, researches each company, and acts as a personal interview coach by automatically generating high-quality technical questions and dynamic explanations tailored to the specific role and the user's experience level.
...
/interview-prep    → Role-based interview sessions, pinned questions
/interview-prep/[id] → Active interview study session with Accordion Learning UI
```
...
```text
Dashboard    Find Jobs    Interview Prep    Profile
```
...
- Configures a new session based on job role and experience level
- Agent uses Gemini API to generate personalized technical questions and answers
- Study session uses Accordion Learning UI to view Q&A cleanly
- Dynamic AI Explanations: User can request on-demand concept breakdowns for complex answers
- Note-taking: User can attach personal notes to specific questions
- Pinning: User pins important questions for quick access later in a dedicated "Pinned" tab

### Dashboard

- Stats bar — 5 cards: Total Jobs Found, Avg. Match Rate, Companies Researched, Jobs This Week, Interview Sessions Completed
...
- Used for job matching and tailoring interview questions
...
- Stored in `interview_sessions` and `interview_questions` tables
- `interview_questions` tracks pinned status, custom notes, and dynamically generated AI explanations
- Leverages Gemini API for dynamic generation
...
- Top navbar — Dashboard, Find Jobs, Interview Prep, Profile
...
- Accordion Learning UI — View Q&A in an expandable UI for a clean study flow
- Dynamic AI Explanations — On-demand concept breakdowns using AI
- Pinning Important Questions — Pin important questions for quick access
- Note-taking on interview questions
...
- Browser extension
- Video or voice interview simulation (text-based Q&A only)
...
interview_session_started; // { userId, role, experienceLevel }
question_pinned; // { userId, questionId }
ai_explanation_requested; // { userId, questionId }
```
...
- Needs to prepare for technical interviews efficiently with tailored questions
...
- Interview prep tool successfully generates accurate, role-specific questions and answers via Gemini API
- Dynamic AI explanations provide useful concept breakdowns
- Users can pin questions and take notes successfully
- Dashboard analytics charts show meaningful data after several searches
- All job data and interview data stored correctly in InsForge with full structured fields
