...
- Navbar — logo, Dashboard, Find Jobs, Interview Prep, Profile links, Start for free button
...
- Session management
- Middleware protecting /dashboard, /profile, /find-jobs, /find-jobs/[id], /interview-prep, /interview-prep/[id]
...
- Create `interview_sessions` and `interview_questions` tables
- Create `agent_logs` table
- Create `resumes` storage bucket with authenticated access only
- Row level security policies on all tables — always filter by user_id
...
## Job Details UI — Company Research Card
...
Build the complete Interview Prep UI with mock data. 

**UI:**

- Interview setup card: input for "Target Role" and dropdown for "Experience Level", Generate button.
- Pinned Questions Tab: view saved questions.
- Active Session View:
  - Header with Target Role and Experience Level.
  - Accordion Learning UI listing 5 technical questions.
  - Expanding an accordion reveals the AI-generated answer.
  - In each expanded item: "Explain Further" button, "Pin" button, and a Textarea for personal notes.
  - Dynamic AI Explanation box (shows up when "Explain Further" is clicked).

---

### 15 Interview Session Generation

Wire up the setup form to generate tailored Q&A via Gemini API.

**Logic:**

- POST /api/agent/interview/generate receives targetRole and experienceLevel.
- Fetch user profile skills and experience from DB to enrich the prompt.
- Call Gemini API to generate 5 tailored technical interview questions and answers.
- Insert a new record in `interview_sessions` table.
- Insert 5 new records in `interview_questions` table.
- Redirect to /interview-prep/[id].
- PostHog event: `interview_session_started`

---

### 16 Dynamic AI Explanations

Wire the "Explain Further" button to fetch on-demand concept breakdowns.

**Logic:**

- POST /api/agent/interview/explain receives questionId.
- Fetch the specific question and answer from DB.
- Call Gemini API to generate a simplified breakdown or step-by-step explanation.
- Update `explanation` column in `interview_questions` table.
- Revalidate page data to show explanation in UI.
- PostHog event: `ai_explanation_requested`

---

### 17 Session Persistence & Notes

Wire up saving notes and pinning questions.

**Logic:**

- Server Action in `actions/interview.ts` to toggle `is_pinned` status.
- Server Action in `actions/interview.ts` to save `notes` text.
- Pinned questions tab fetches all `interview_questions` where `is_pinned = true`.
- PostHog event: `question_pinned`
...
- Five stat cards: Total Jobs Found, Avg. Match Rate, Companies Researched, Jobs This Week, Interview Sessions Completed — all showing mock numbers with trend indicators
...
Wire five stat cards to real InsForge DB data for current user.
...
- Interview Sessions Completed — COUNT of interview_sessions for current user
...
- Query interview_sessions table — most recent sessions for current user
...
- Format each into human readable string:
  ...
  - interview_session started → "Prepared for [targetRole] interview — [time ago]"
...
| Phase 5 — Interview Preparation | 4        |
| Phase 6 — Dashboard             | 4        |
| **Total**                       | **21**   |
