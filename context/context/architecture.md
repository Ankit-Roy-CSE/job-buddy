...
| AI model                       | Google Gemini            | Interview Q&A generation, concept explanations   |
...
│   ├── interview-prep/
│   │   ├── page.tsx                       → Role-based interview sessions, pinned questions
│   │   └── [id]/
│   │       └── page.tsx                   → Active interview study session with Accordion UI
│   └── api/
│       ├── agent/
│       │   ├── find/route.ts              → Trigger Adzuna job discovery
│       │   ├── research/route.ts          → Trigger company research agent
│       │   └── interview/
│       │       ├── generate/route.ts      → Generate Q&A via Gemini API
│       │       └── explain/route.ts       → On-demand AI concept breakdowns
│       ├── resume/
...
│   ├── interview.ts                       → Gemini API integration for generating Q&A and explanations
│   └── types.ts                           → Agent-specific TypeScript types
├── actions/
│   ├── profile.ts                         → Profile save + update
│   ├── jobs.ts                            → Job status updates
│   └── interview.ts                       → Save interview sessions, pin questions, save notes
├── components/
...
| `agent/`      | All agent logic. Adzuna discovery, company research, matching, extraction, interview prep Q&A. Nothing here touches React. |
| `actions/`    | Server Actions for UI-triggered mutations only. Profile save, profile update, pinning questions.       |
...
### Interview Preparation (API Routes & Server Actions)

```
User configures session and clicks Generate
        ↓
API route in app/api/agent/interview/generate
        ↓
Calls agent/interview.ts (Gemini API)
        ↓
Generates tailored technical Q&A
        ↓
Saves to interview_sessions and interview_questions in InsForge DB
        ↓
Page redirects to session view

User requests concept explanation on a specific question
        ↓
API route in app/api/agent/interview/explain
        ↓
Calls agent/interview.ts (Gemini API)
        ↓
Generates and returns dynamic explanation
```
...
| question         | text        |                                          |
| answer           | text        |                                          |
| explanation      | text        | Optional dynamically generated breakdown |
...
- Protected routes: /dashboard, /profile, /find-jobs, /find-jobs/[id], /interview-prep, /interview-prep/[id]
- Public routes: /, /login
...
## Interview Prep Generation Pattern

**Gemini API — tailored Q&A generation**
...
const prompt = `Generate 5 technical interview questions and answers for a ${experienceLevel} ${targetRole}. Return as JSON array.`;
const result = await model.generateContent(prompt);
const response = await result.response;
const text = response.text();
// Parse JSON text into structured Q&A and save to DB
```
